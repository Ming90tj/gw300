...
//	genvar byte_index;     
    ...
    assign     fifo_rden_wire =fifo_rden;
	...
	begin
		iq_next <= 1'b0;
			...
			if(iq_start_wire)begin
				...
				else if(status == 5'd2)begin
						iq_txd[0] <= iq_buffer[i_base + status];
						iq_txd[1] <= iq_buffer[q_base + status];
						status 	  <= status + 1;
						end
				else if(status == 5'd3)begin
						iq_txd[0] <= iq_buffer[i_base + status];
						iq_txd[1] <= iq_buffer[q_base + status];
						status 	  <= status + 1;
						end				
				else if(status == 5'd4)begin
						iq_txd[0] <= iq_buffer[i_base + status];
						iq_txd[1] <= iq_buffer[q_base + status];
						status 	  <= status + 1;
						end				
				else if(status == 5'd5)begin
						iq_txd[0] <= iq_buffer[i_base + status];
						iq_txd[1] <= iq_buffer[q_base + status];
						status 	  <= status + 1;
						end					
				else if(status == 5'd6)begin
						iq_txd[0] <= iq_buffer[i_base + status];
						iq_txd[1] <= iq_buffer[q_base + status];
						status 	  <= status + 1;
						end				
				else if(status == 5'd7)begin
						iq_txd[0] <= iq_buffer[i_base + status];
						iq_txd[1] <= iq_buffer[q_base + status];
						status 	  <= status + 1;
						end				
				else if(status == 5'd8)begin
						iq_txd[0] <= iq_buffer[i_base + status];
						iq_txd[1] <= iq_buffer[q_base + status];
						status 	  <= status + 1;
						end				
				else if(status == 5'd9)begin
						iq_txd[0] <= iq_buffer[i_base + status];
						iq_txd[1] <= iq_buffer[q_base + status];
						status 	  <= status + 1;
						end				
				else if(status == 5'd10)begin
						iq_txd[0] <= iq_buffer[i_base + status];
						iq_txd[1] <= iq_buffer[q_base + status];
						status 	  <= status + 1;
						end				
				else if(status == 5'd11)begin
						iq_txd[0] <= iq_buffer[i_base + status];
						iq_txd[1] <= iq_buffer[q_base + status];
						status 	  <= status + 1;
						end				
				else if(status == 5'd12)begin
						iq_txd[0] <= iq_buffer[i_base + status];
						iq_txd[1] <= iq_buffer[q_base + status];
						status 	  <= status + 1;
						end				
				else if(status == 5'd13)begin
						iq_txd[0] <= iq_buffer[i_base + status];
						iq_txd[1] <= iq_buffer[q_base + status];
						status 	  <= status + 1;
						end				
				else if(status == 5'd14)begin
						iq_txd[0] <= iq_buffer[i_base + status];
						iq_txd[1] <= iq_buffer[q_base + status];
						status 	  <= status + 1;
						end				
				else if(status == 5'd15)begin
						iq_txd[0] <= iq_buffer[i_base + status];
						iq_txd[1] <= iq_buffer[q_base + status];
						if(!fifo_rden_wire)begin
							status	<= status + 1;
							end
						else
							status	<= 5'd0;
